...
----------------------- | ------------------------ | ------------- |
AST Evaluation          | ~ 6 μs                   | ~ 4 μs        |
Fibonacci               | ~ 2 μs                   | ~ 2 μs        |
List Processing         | ~ 87 μs                  | ~ 1 μs        |
Tail Call Optimization  | ~ 990 μs                 | ~ 56 μs       |
Deep Record Updates     | ~ 1313 μs                | ~ 3 μs        |
Ackermann               | ~ 34 μs                  | ~ 32 μs       |
Church Numerals         | ~ 7391 μs                | ~ 34 μs       |
Prime Sieve             | ~ 261 μs                 | ~ 3 μs        |
Red-Black Tree          | ~ 27163 μs               | ~ 24154 μs    |
Polymorphism            | ~ 2357 μs                | ~ 2387 μs     |
State Monad             | ~ 136 μs                 | ~ 1 μs        |
Lazy Evaluation         | ~ 19762 μs               | ~ 305 μs      |
Array Processing        | ~ 57 μs                  | ~ 1 μs        |
RowToList               | ~ 1 μs                   | ~ 1 μs        |
----------------------- | ------------------------ | ------------- |
Total Execution Time    | ~ 59.56 ms               | ~ 26.98 ms    | -> Read the IMPORTANT notice below!
...
>
> The Lazy Evaluation benchmark is the perfect example of this: the PureScript code is deliberately designed to be as inefficient as possible (dynamically allocating and forcing 1 million closures to heavily stress the garbage collector and call stack). In contrast, the FFI native version completely short-circuits this functional abstraction, replacing it with a brutal, hyper-optimized imperative `for` loop. That's unfair, but that is the goal: see what happens when a developer makes a huge design mistake, and what's the ratio, at worst.
>
