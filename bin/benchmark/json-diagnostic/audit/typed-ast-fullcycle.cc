...
// Audit counters: which share of the type table is actually built. The C++
// reference resolves lazily; the Go decoder resolves every entry eagerly.
size_t g_type_builds = 0;
size_t g_type_entries = 0;
...
class Arena {
 public:
  ...
  // Audit hook: release the backing blocks so the cost of freeing the decoded
  // structures can be measured explicitly (DIAG_C_FULL_CYCLE). The default
  // path never calls this and behaves exactly like typed-ast.cc.
...
Module decode_module(Arena &arena, element root, std::string_view debug_name) {
  ...
  g_type_entries += raw_entries.size();
...
int main() {
  ...
  const size_t type_builds = g_type_builds;
  const size_t type_entries = g_type_entries;
  const bool full_cycle = getenv("DIAG_C_FULL_CYCLE") != nullptr;
  const char *phase_names[3] = {"parse", "decode", "combined"};
  std::string phases = "{";
  for (int phase = 0; phase < 3; phase++) {
    double best = 0;
    double best_release = 0;
    ...
    for (int pass = 0; pass < 7; pass++) {
      ...
      if (full_cycle) {
        // The Go interval contains collector work; price the matching free.
        double release_begin = now_microseconds();
        arena.release();
        modules.clear();
        modules.shrink_to_fit();
        double release_elapsed = now_microseconds() - release_begin;
        if (pass >= 2 && (!have_release || release_elapsed < best_release)) {
          best_release = release_elapsed;
          have_release = true;
        }
      }
      if (pass >= 2) {
    ...
    phases += json_quoted(phase_names[phase]) + ":{\"samples\":" + samples +
              ...
              ",\"release_us\":" + std::to_string(best_release) + "}";
    if (phase < 2) phases += ",";
  }
  phases += "}";
  ...
  report += "],\"type_builds\":" + std::to_string(type_builds) +
            ",\"type_entries\":" + std::to_string(type_entries) +
            ",\"phases\":" + phases + "}";
