...
let validate (value: obj) =
    ...
    if actual <> "10000000" then failwithf "Polymorphism: expected 10000000, got %A" actual
    actual

let measure () =
    // Resolve the real action before timing. Its opaque input, public wrapper,
    // effects and conversion to String remain part of every invocation.
    let act = unbox<obj -> obj> Test_Polymorphism_act
    ...
    emit {| kind = "metadata"; runtime = RuntimeInformation.FrameworkDescription
            ...
            action = "Test.Polymorphism.act"; input = 10000000; pid = Environment.ProcessId
    ...
    emit {| kind = "summary"; bestUs = times.[0]; medianUs = (times.[4] + times.[5]) / 2.0
            ...
            gc2 = measurements |> Array.sumBy (fun item -> item.gc2); output = "10000000" |}
