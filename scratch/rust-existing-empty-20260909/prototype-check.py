...
after = (HERE/'RBTree-prototype.rs').read_text()
...
fn counted(value: Tree) -> std::rc::Rc<Tree> {
    match &value {
        Tree::E => { EMPTY.fetch_add(1, Ordering::Relaxed); }
        Tree::T(..) => { NODES.fetch_add(1, Ordering::Relaxed); }
    }
    std::rc::Rc::new(value)
}
struct Counting;
...
fn main() {
    correctness();
    retained_versions();
    for counter in [&ALLOCS, &FREES, &BYTES, &SMALL, &EMPTY, &NODES] { counter.store(0, Ordering::Relaxed); }
    let tree = Test_RBTree_buildTree(std::hint::black_box(100000), counted(Tree::E));
    ...
    assert_eq!(allocs, empty + nodes);
    println!("{} {} {} {} {} {} {}", allocs, frees, bytes, small, std::mem::size_of::<Tree>(), empty, nodes);
}
'''
...
for side, generated in [('before', before), ('after', after)]:
    ...
    source.write_text('#![allow(warnings)]\n'+kernel.replace('std::rc::Rc::new(', 'counted(')+harness)
    ...
    results[side] = dict(zip(['allocations', 'deallocations', 'requested_bytes', 'allocations_24_bytes', 'tree_size', 'empty_constructors', 'payload_constructors'], values))
...
(HERE/'prototype-allocations.json').write_text(json.dumps(results, indent=2)+'\n')
