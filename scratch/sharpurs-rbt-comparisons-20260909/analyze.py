...
from common import AUDIT, ORDER, RBT, baseline, check_outputs, compare, read_run, require, sha, stats
...
for variant, summary in summaries.items():
    total, rbt = summary["total_ms"], summary["tests_us"][RBT]
...
for label, change in (("Total", changes["total_ms"]), ("RBTree", changes["tests_us"][RBT])):
...
print(f"Official README: {official['compiled_total_ms']:.2f} ms total; "
      f"RBTree compiled {official['compiled_rbtree_us'] / 1000:.3f} ms, "
      f"native optimized {official['native_optimized_rbtree_us'] / 1000:.3f} ms (different data structure/result).")
