...
def verify_sources():
    require(inputs["afterSha256"] == inputs["sourceGeneratedSha256"],
            "Integration after snapshot must equal the normal generated sources")
    require(inputs["officialReadmeSha256"] == official["sha256"], "Official README snapshot changed")
    for directory, expected in [(AUDIT / "before", inputs["beforeSha256"]),
        ...
        actual_names = {path.name for path in directory.iterdir() if path.is_file()
                        and path.suffix in {".fs", ".cs", ".fsproj", ".csproj", ".props"}}
        require(actual_names == set(expected), f"Generated source file set changed: {directory}")
