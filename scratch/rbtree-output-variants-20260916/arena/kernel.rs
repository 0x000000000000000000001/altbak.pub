// Isolated, safe arena lowering of the generated Okasaki algorithm.
...
struct Arena { nodes: Vec<Node>, root: Index }
impl Arena {
    ...
    fn balance(&mut self, root: Index) -> Index {
        let node = self.nodes[root as usize];
        if node.red { return root; }
        let direction = if self.red(node.left) {
