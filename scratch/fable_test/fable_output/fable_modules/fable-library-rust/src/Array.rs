pub mod Array_ {
    ...
    use crate::module_3bd9ae6a::Native_::makeCompare;
    ...
    use crate::module_6dcf8b93::NativeArray_::new_copy;
    ...
    use crate::module_8d7d6be8::Option_::defaultArg;
    ...
    pub fn compareWith<T: Clone + 'static>(
        ...
        while if res.get() == 0_i32 {
            i.get() < len
        } else {
            false
        } {
            res.set(comparer(source1[i.get()].clone(), source2[i.get()].clone()));
            i.set(i.get() + 1_i32)
        }
        if res.get() != 0_i32 {
            res.get()
        ...
        if len1 > len2 {
            ...
            if len1 < len2 {
                ...
                while if res.get() == 0_i32 {
                    ...
                    res.set(compare(source1[i.get()].clone(), source2[i.get()].clone()));
                    i.set(i.get() + 1_i32)
                }
                res.get()
            }
    ...
    pub fn equals<T: Eq + core::hash::Hash + Clone + 'static>(
        ...
        if len1 == count_2(source2.clone()) {
            ...
            while if res.get() { i.get() < len1 } else { false } {
                res.set(source1[i.get()].clone() == source2[i.get()].clone());
                i.set(i.get() + 1_i32)
            }
            res.get()
        } else {
    ...
    pub fn mapFold<State: Clone + 'static, T: Clone + 'static, U: Clone + 'static>(
        ...
        let res: Array<U> = new_with_capacity::<U>(len);
        for i in 0_i32..=len - 1_i32 {
            let m: LrcPtr<(U, State)> = mapping(acc.get(), source[i].clone());
            add(res.clone(), m.0.clone());
            acc.set(m.1.clone())
        }
        LrcPtr::new((res.clone(), acc.get()))
    }
    pub fn mapFoldBack<T: Clone + 'static, State: Clone + 'static, U: Clone + 'static>(
        ...
        for i in (0_i32..=len - 1_i32).rev() {
            let m: LrcPtr<(U, State)> = mapping(source[i].clone(), acc.get());
            add(res.clone(), m.0.clone());
            acc.set(m.1.clone())
        }
        res.get_mut().reverse();
    ...
    pub fn concat<T: Clone + 'static>(sources: Array<Array<T>>) -> Array<T> {
        ...
        for idx in 0_i32..=count_2(sources.clone()) - 1_i32 {
            ...
            len.set(len.get() + count_2(arr))
        }
        {
            let res: Array<T> = new_with_capacity::<T>(len.get());
    ...
    pub fn exists<T: Clone + 'static>(predicate: Func1<T, bool>, source: Array<T>) -> bool {
        ...
        while if i.get() < count_2(source.clone()) {
            !res.get()
        } else {
            false
        } {
    ...
    pub fn exists2<T1: Clone + 'static, T2: Clone + 'static>(
        ...
        }
        {
            let i: MutCell<i32> = MutCell::new(0_i32);
            let res: MutCell<bool> = MutCell::new(false);
            while if i.get() < count_2(source1.clone()) {
                !res.get()
            } else {
                false
            } {
    ...
    pub fn skipWhile<T: Clone + 'static>(predicate: Func1<T, bool>, source: Array<T>) -> Array<T> {
        ...
        while if count.get() < count_2(source.clone()) {
        ...
        Array_::getSubArray(
            source.clone(),
            count.get(),
            count_2(source.clone()) - count.get(),
        )
    }
    ...
    pub fn takeWhile<T: Clone + 'static>(predicate: Func1<T, bool>, source: Array<T>) -> Array<T> {
        ...
        Array_::getSubArray(source.clone(), 0_i32, count.get())
    }
    ...
    pub fn tryFind<T: Clone + 'static>(predicate: Func1<T, bool>, source: Array<T>) -> Option<T> {
        fn inner_loop<T: Clone + 'static>(
            ...
            '_inner_loop: loop {
                break '_inner_loop (if i.get() >= count_2(source_1.get()) {
                    None::<T>
                } else {
                    if predicate_1(source_1[i.get()].clone()) {
                        Some(source_1[i.get()].clone())
                    } else {
                        let i_temp: i32 = i.get() + 1_i32;
                        let predicate_1_temp = predicate_1.get();
                        let source_1_temp: Array<T> = source_1.get();
                        i.set(i_temp);
    ...
    pub fn tryFindBack<T: Clone + 'static>(
        ...
        fn inner_loop<T: Clone + 'static>(
            ...
            let source_1: MutCell<Array<T>> = MutCell::new(source_1.clone());
            '_inner_loop: loop {
                break '_inner_loop (if i.get() < 0_i32 {
                    None::<T>
                } else {
                    if predicate_1(source_1[i.get()].clone()) {
                        Some(source_1[i.get()].clone())
        ...
        fn inner_loop<T: Clone + 'static, U: Clone + 'static>(
            ...
            '_inner_loop: loop {
                break '_inner_loop (if i.get() >= count_2(source_1.get()) {
                    ...
                    let matchValue: Option<U> = chooser_1(source_1[i.get()].clone());
    ...
    pub fn fold2<State: Clone + 'static, T1: Clone + 'static, T2: Clone + 'static>(
        ...
        for i in 0_i32..=count_2(source1.clone()) - 1_i32 {
            acc.set(folder(acc.get(), source1[i].clone(), source2[i].clone()));
        }
        acc.get()
    }
    ...
    pub fn unfold<State: Clone + 'static, T: Clone + 'static>(
        ...
        fn inner_loop<State: Clone + 'static, T: Clone + 'static>(
            ...
            '_inner_loop: loop {
                break '_inner_loop ({
                    let matchValue: Option<LrcPtr<(T, State)>> = generator_1(state_1.get());
                    match &matchValue {
                        Some(matchValue_0_0) => {
                            let s: State = (matchValue_0_0).1.clone();
                            add(res.get(), (matchValue_0_0).0.clone());
                            {
                                let generator_1_temp = generator_1.get();
                                let state_1_temp: State = s.clone();
                                let res_temp: Array<T> = res.get();
    ...
    pub fn sum<T: core::ops::Add<Output = T> + Default + Clone + 'static>(source: Array<T>) -> T {
        ...
        for i in 0_i32..=count_2(source.clone()) - 1_i32 {
            acc.set(acc.get() + source[i].clone());
        }
        acc.get()
    }
    pub fn sumBy<T: Clone + 'static, U: core::ops::Add<Output = U> + Default + Clone + 'static>(
        ...
        for i in 0_i32..=count_2(source.clone()) - 1_i32 {
            acc.set(acc.get() + projection(source[i].clone()));
        }
        acc.get()
    }
    ...
    >(
        ...
        }
        {
            ...
            for i in 0_i32..=count_2(source.clone()) - 1_i32 {
                total.set(total.get() + source[i].clone());
            }
            total.get() / T::from(count_2(source.clone()))
        }
    }
    pub fn averageBy<
        T: Clone + 'static,
    ...
    >(
        ...
        }
        {
            ...
            for i in 0_i32..=count_2(source.clone()) - 1_i32 {
                total.set(total.get() + projection(source[i].clone()));
            }
            total.get() / U::from(count_2(source.clone()))
        }
    }
