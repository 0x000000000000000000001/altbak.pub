...
from common import AUDIT, ORDER, RBT, LAZY, baseline, check_outputs, read_run, require, sha
...
official = baseline()
configs, assemblies, builds = {}, {}, {}
require(inputs["sourceGeneratedSha256"] == inputs["beforeSha256"],
        "Normal generated sources must match before in this isolated experiment")

require(set(inputs["beforeSha256"]) == set(inputs["afterSha256"]), "Snapshot file sets differ")
require([name for name, digest in inputs["beforeSha256"].items()
         if digest != inputs["afterSha256"][name]] == ["Test.RBTree.fs"],
        "Expected only Test.RBTree.fs to differ")
...
def verify_sources():
    for directory, expected in [(AUDIT / "before", inputs["beforeSha256"]),
        ...
        require(len(expected) == 355, f"Expected exactly 355 source inputs for {directory}")
        actual_names = {p.name for p in directory.iterdir()
                        if p.is_file() and p.suffix in {".fs", ".cs", ".fsproj", ".csproj", ".props"}}
        require(actual_names == set(expected), f"Source file set changed: {directory}")
        for name, expected_hash in expected.items():
            require(sha(directory / name) == expected_hash, f"Source changed: {directory / name}")
    for name, expected_hash in inputs["sourceRuntimeSha256"].items():
        require(sha(source / "bin/Release/net8.0" / name) == expected_hash,
                f"Normal runtime changed: {name}")
    backend = source.parents[5] / "sharpurs/sharpurs"
    require(sha(backend / "bin/sharpurs.js") == inputs["backendBundleSha256"],
            "Backend bundle changed during experiment")
    for name, expected_hash in inputs["backendSourcesSha256"].items():
        require(sha(backend / name) == expected_hash, f"Backend source changed: {name}")
    require(sha(inputs_path) == manifest_sha, "inputs.json changed during measurements")
    for variant, build in builds.items():
        require(build["sourceSha256"] == inputs[f"{variant}Sha256"], "Build source manifest mismatch")
        require(sha(AUDIT / f"build-{variant}.log") == build["buildLogSha256"], "Build log changed")
        for name, expected in build["runtimeSha256"].items():
            require(sha(AUDIT / variant / "bin/Release/net8.0" / name) == expected,
                    f"Runtime file changed: {variant}/{name}")
...
for variant in ("before", "after"):
    ...
    builds[variant] = json.loads((AUDIT / f"build-{variant}.json").read_text())
...
metadata = {"startedUtc": utc(), "probe": args.probe, "order": order,
            ...
            "buildManifestsSha256": {variant: sha(AUDIT / f"build-{variant}.json") for variant in builds},
...
for variant in order:
    ...
    summary = {"total_ms": run["total_ms"], "lazy_ms": next(
        test["best_of_10_us"] / 1000 for test in run["tests"] if test["name"] == LAZY), "rbtree_ms": next(
