...
// Test.Polymorphism binding, whether the current Sharpurs converters accept the
...
await run(Builder.buildModules(Aff.monadAff)({
  ...
  onCodegenModule: (_) => (corefn) => (module) => (_) => {
    ...
    if (module.name === "Test.Polymorphism") {
      if (captured) throw new Error("Duplicate Test.Polymorphism module");
      captured = { corefn, module };
    }
    return pure(undefined);
  },
})(modules));
if (!captured) throw new Error("Optimized Test.Polymorphism not found");
...
console.log(`Inputs sha256: output/Test.Polymorphism/corefn.json ${createHash("sha256").update(readFileSync(resolve(benchmark, "output/Test.Polymorphism/corefn.json"))).digest("hex").slice(0, 12)}`);
