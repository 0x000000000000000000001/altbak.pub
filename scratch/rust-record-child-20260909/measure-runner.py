...
label = 'confirmation' if args.confirmation else 'paired'
result_file = 'runner-confirmation.json' if args.confirmation else 'runner-results.json'
...
for pair in range(5):
    for side in (['before', 'after'] if pair % 2 == 0 else ['after', 'before']):
        ...
        (HERE/f'build/{label}-{side}-{pair+1}.log').write_text(output)
        values = parse(output)
...
(HERE/result_file).write_text(json.dumps(result, indent=2)+'\n')
