#!/usr/bin/env python3
"""Summarize paired measurements; never run the compiler or benchmarks.
...
def load():
    ...
    rounded_zero_times = []
    lines = 0
    for line_no, line in enumerate(Path("measures.jsonl").read_text().splitlines(), 1):
        if not line.strip():
            continue
        row = json.loads(line)
    ...
    summary = {
        "schema_version": 1, "input": "measures.jsonl", "measurement_lines": lines,
        "statistics": {
...
def markdown(summary):
    ...
    table("Noyau sumEvens", summary["core"].items(), costs)
...
def main():
    summary = load()  # Validate all measurements before writing either output.
    rendered = markdown(summary)
    Path("summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False,
                                               allow_nan=False) + "\n")
    Path("summary.md").write_text(rendered)
    print(f"Validated {summary['measurement_lines']} measurement lines; wrote summary.json and summary.md.")
