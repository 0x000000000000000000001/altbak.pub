...
await run(Builder.buildModules(Aff.monadAff)({
  ...
  onCodegenModule: (_) => (corefn) => (module) => (_) => {
    count++;
    if (module.name === "Test.Polymorphism") {
      if (found) throw new Error("Duplicate Test.Polymorphism module");
      found = true;
      ...
      for (const group of module.bindings) {
        for (const binding of group.bindings) {
          ...
          console.log(`Captured optimized Test.Polymorphism.${binding.value0}`);
        }
      }
    }
...
if (!found) throw new Error("Optimized Test.Polymorphism not found");
for (const name of ["polyLoop", "act", "intMonoidish"]) {
...
const inputs = [
  resolve(benchmark, "output/Test.Polymorphism/corefn.json"),
