...
fn main() {
    ...
    assert!(matches!(run(&for_loop), Value::Record_a(_)));
    run(&for_loop);
    ...
    assert!(log.borrow().is_empty());
    assert!(matches!(run(&foreach_loop), Value::Record_a(_)));
    run(&foreach_loop);
    ...
    assert!(matches!(run(&until), Value::Record_a(_)));
    ...
    assert!(matches!(run(&while_loop), Value::Record_a(_)));
