...
await run(Builder.buildModules(Aff.monadAff)({
  ...
  onCodegenModule: (_) => (corefn) => (module) => (_) => {
    count++;
    if (module.name === "TypeAppFixture") {
      if (found) throw new Error("Duplicate TypeAppFixture module");
      found = true;
      ...
      for (const group of module.bindings) {
        for (const binding of group.bindings) {
          ...
          console.log(`Captured optimized TypeAppFixture.${binding.value0}`);
        }
      }
    }
...
if (!found) throw new Error("Optimized TypeAppFixture not found");
for (const name of ["identity", "useInt", "useString", "generic"]) {
...
const inputs = [
  resolve(audit, "TypeAppFixture.purs"),
  resolve(backend, "output/PureScript.Backend.Optimizer.CoreFn/index.js"),
  resolve(audit, "output/TypeAppFixture/corefn.json"),
  ...
  ...["Builder", "App", "Convert", "Semantics", "TypeSubstitution", "Syntax", "Semantics.Foreign"]
    .map((name) => resolve(backend, `output/PureScript.Backend.Optimizer.${name}/index.js`)),
  ...["Convert", "Semantics", "TypeSubstitution"]
    .map((name) => resolve(pbo, `src/PureScript/Backend/Optimizer/${name}.purs`)),
  resolve(pbo, "test/typeapp.mjs"),
];
