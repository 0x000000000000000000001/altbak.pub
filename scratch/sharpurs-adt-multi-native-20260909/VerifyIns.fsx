...
let paths =
    ["public", publicIns; "bridge", bridgeIns
     ...
     "native guarded", Test_RBTree_ins_adt_native_apply
#endif
    ]
...
#if BASELINE
#else
// New compiler-selected insert/buildTree helpers, as actually generated.
for value in [Int32.MinValue; -1; 0; 2; 4; Int32.MaxValue] do
    for input in [empty; seed] do
        let expected = insert value input
        check "native insert agrees with public wrapper" (Test_RBTree_insert_adt_native value input = expected)
        check "guarded native insert agrees" (Test_RBTree_insert_adt_native_apply value input = expected)
for count in [0; 1; 33; 1000] do
    for input in [empty; seed] do
        let expected = unbox<Test_RBTree_Tree> (sharpurs_apply (sharpurs_apply Test_RBTree_buildTree (box count)) (box input))
        check "native buildTree agrees with public wrapper" (Test_RBTree_buildTree_adt_native count input = expected)
        check "guarded native buildTree agrees" (Test_RBTree_buildTree_adt_native_apply count input = expected)
        check "recursive buildTree ABI agrees" (unbox<Test_RBTree_Tree> (Test_RBTree_buildTree_tco (box count) (box input)) = expected)
        if count = 0 then check "zero buildTree shares its input" (Object.ReferenceEquals(input, expected))
#endif
