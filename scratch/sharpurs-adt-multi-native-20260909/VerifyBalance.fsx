...
#if BASELINE
let rawMethodName = "Test_RBTree_balance_direct"
let applyMethodName = "Test_RBTree_balance_direct_apply"
let parameterTypes = [| typeof<obj>; typeof<obj>; typeof<obj>; typeof<obj> |]
let resultType = typeof<obj>
#else
let rawMethodName = "Test_RBTree_balance_adt_native"
let applyMethodName = "Test_RBTree_balance_adt_native_apply"
let parameterTypes = [| typeof<Test_RBTree_Color>; typeof<Test_RBTree_Tree>; typeof<int>; typeof<Test_RBTree_Tree> |]
let resultType = typeof<Test_RBTree_Tree>
#endif
...
let directMethods =
    ...
    |> Array.filter (fun methodInfo -> methodInfo.Name = rawMethodName)
...
check "raw method has exact generated parameter types" ((directParameters |> Array.map (fun parameter -> parameter.ParameterType)) = parameterTypes)
check "raw method has exact generated return type" (directMethod.ReturnType = resultType)

let applyMethods =
    moduleType.GetMethods(Reflection.BindingFlags.Public ||| Reflection.BindingFlags.Static)
    |> Array.filter (fun methodInfo -> methodInfo.Name = applyMethodName)
check "one public static direct application method" (applyMethods.Length = 1)
let applyMethod = applyMethods.[0]
check "guarded method has exact generated parameter types"
    (applyMethod.GetParameters().Length = 4
     && ((applyMethod.GetParameters() |> Array.map (fun parameter -> parameter.ParameterType)) = parameterTypes))
check "guarded method has exact generated return type" (applyMethod.ReturnType = resultType)
...
#else
    Test_RBTree_balance_adt_native color left value right
#endif
...
#else
    Test_RBTree_balance_adt_native_apply color left value right
#endif
