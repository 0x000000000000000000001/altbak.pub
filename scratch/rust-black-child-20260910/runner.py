...
NAMES = ['before','black_rebuild','black_field']
...
def build():
    ...
    versions = {'before':original,
        'black_rebuild':original.replace(signature,signature+probe.GUARD.replace('BODY',probe.REBUILD)),
        'black_field':original.replace(signature,signature+probe.GUARD.replace('BODY',probe.FIELD))}
