...
after = (GENERATED/'Purs_Test_RBTree/src/lib.rs').read_text()
...
for side, generated in [('before', before), ('after', after)]:
    ...
    source = HERE/f'rbtree-{side}-checks.rs'
    binary = HERE/f'rbtree-{side}-checks'
...
results['invariants'] = 'Four rotation cases, ordering, black root, black heights, red children, duplicates, old root after insertion and destruction all checked on both versions.'
