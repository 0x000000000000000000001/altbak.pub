...
await run(Builder.buildModules(Aff.monadAff)({
  ...
  onCodegenModule: (_) => (core) => (backendModule) => (_) => {
    const code = Printer.printModule(CodeGen.translateOptimizedModule(arities)(backendModule)(core));
    const old = readFileSync(resolve(benchmark, "run/bak/sharp/output/Main", backendModule.name + ".fs"), "utf8");
...
if (changed.length) throw new Error("Default generation changed: " + changed.join(", "));
console.log(`${checked.length} module bodies match the existing generated benchmark exactly.`);
