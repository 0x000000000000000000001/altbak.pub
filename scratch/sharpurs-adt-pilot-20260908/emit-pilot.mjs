...
const AdtKernel = await load("Sharpurs.AdtKernel");
const Printer = await load("Sharpurs.Printer");
...
await run(Builder.buildModules(Aff.monadAff)({
  ...
  onCodegenModule: (_) => (corefn) => (module) => (_) => {
    ...
    if (module.name === "AdtPilot") {
      ...
      found = true;
      const native = AdtKernel.fromModule(corefn)(module);
      if (!(native instanceof Maybe.Just)) throw new Error("ADT pilot rejected");
      writeFileSync(resolve(audit, "AdtPilot.generated.fs"), 'module AdtPilotGenerated\n\n' + Printer.printModule(native.value0));
