...
const generated = resolve(benchmark, "run/bak/sharp/output/Main");
...
await run(Builder.buildModules(Aff.monadAff)({
  ...
  onCodegenModule: (_) => (corefn) => (module) => (_) => {
    ...
    if (module.name === "Test.RBTree") {
      if (found) throw new Error("Duplicate Test.RBTree module");
      found = true;
      ...
      for (const group of module.bindings) {
        for (const binding of group.bindings) {
          ...
          console.log(`Captured optimized Test.RBTree.${binding.value0}`);
        }
      }
    }
...
if (!found) throw new Error("Optimized Test.RBTree not found");
for (const name of ["depth", "makeBlack", "ins", "balance", "max", "act"]) {
...
const inputs = [
  resolve(benchmark, "src/Test/RBTree.purs"),
  resolve(backend, "output/PureScript.Backend.Optimizer.CoreFn/index.js"),
  resolve(benchmark, "run/bak/sharp/output/Test.RBTree/corefn.json"),
...
writeFileSync(resolve(audit, "ir-metadata.json"), encode({
