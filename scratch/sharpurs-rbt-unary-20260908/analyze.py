...
SPEC = importlib.util.spec_from_file_location(
    "regression_analysis", DIRECTORY / "common.py"
)
...
def read_diagnostic(path):
    ...
    require(
        all(item.get("output") == "22" for item in warmups + measurements + [summary]),
        f"{path}: incorrect RBTree output",
    )
...
def main():
    ...
    baseline = COMMON.read_readme_baseline(DIRECTORY / "README.baseline.md", names)
    require(baseline["tests_us"][RBT] == 213760, "README RBTree baseline changed; review assumptions")
    ...
    results = {
        "validation": {
            ...
            "diagnostic_outputs_all_equal_22": True,
        ...
        "method": {
            ...
            "diagnostic": "True Test.RBTree.act after 3 App.warmup and 3 local warmups; 10 actions per fresh process, 3 processes per variant.",
        ...
        "after_vs_official_readme": {
            ...
            "rbtree_us": compare(baseline["tests_us"][RBT], after["tests_us"][RBT]["median"]),
        },
    }
    ...
    for variant in ("before", "after"):
        ...
        poly = official_stats[variant]["tests_us"][RBT]
        diag = diagnostic_summaries[variant]
        allocations = diag["allocation_bytes_per_action"]
        print(f"{variant}: official total median {total['median']:.2f} ms [{total['min']:.2f}, {total['max']:.2f}], RBTree {poly['median'] / 1000:.5f} ms")
    ...
    for label, change in (("Total", official_changes["total_ms"]), ("RBTree", official_changes["tests_us"][RBT])):
        print(f"Official {label}: {change['difference_percent']:+.2f}%, {change['speedup_factor']:.2f}× speedup")
    print(f"README historical compiled F#/C#: {baseline['total_ms']:.2f} ms total, {baseline['tests_us'][RBT]} µs RBTree")
