...
(audit / ("probe-metadata.json" if probe else "metadata.json")).write_text(json.dumps(metadata, indent=2) + "\n")

def execute(kind, variant, number):
    label = ("probe-" if probe else "") + f"{kind}-{variant}-{number}"
    ...
    if kind == "official":
        ...
        assert all(row["output"] == "22" for row in measured + warmups)
    ...
    (audit / ("probe-executions.json" if probe else "executions.json")).write_text(json.dumps(executions, indent=2) + "\n")
    print(json.dumps({"label": label, "wallSeconds": elapsed, **summary}), flush=True)

orders = [("official", ["before", "after"])] if probe else [
...
assert all(sha(root / "run/bak/sharp/output/Main" / name) == value for name, value in inputs["afterSha256"].items())
metadata["generatedSourcesUnchangedDuringMeasurements"] = True
metadata["finishedUtc"] = datetime.now(timezone.utc).isoformat()
(audit / ("probe-metadata.json" if probe else "metadata.json")).write_text(json.dumps(metadata, indent=2) + "\n")
print("Probe complete: one official process per variant." if probe else "Complete: ten official runs, six diagnostics, generated sources unchanged during measurements.", flush=True)
