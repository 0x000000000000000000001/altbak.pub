...
let validate (value: obj) =
    ...
    if actual <> "22" then failwithf "RBTree: expected 22, got %A" actual
    actual
...
let measure () =
    ...
    let act = unbox<obj -> obj> Test_RBTree_act
    ...
    emit {| kind = "metadata"; runtime = RuntimeInformation.FrameworkDescription
            ...
            action = "Test.RBTree.act"; input = 100000; pid = Environment.ProcessId
    ...
    emit {| kind = "summary"; bestUs = times.[0]; medianUs = (times.[4] + times.[5]) / 2.0
            ...
            gc2 = measurements |> Array.sumBy (fun item -> item.gc2); output = "22" |}
