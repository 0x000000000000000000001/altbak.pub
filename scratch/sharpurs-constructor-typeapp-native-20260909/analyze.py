...
from common import AUDIT, ORDER, RBT, LAZY, PRIMES, LIST, baseline, check_outputs, compare, read_run, require, sha, stats
...
results = {"validation": {"passed": True, "official_processes_per_variant": count,
           ...
           "method": {"official": f"Unmodified App.main entry point: best of 10 per test, {count} fresh processes per variant.",
                      ...
                      "caution": "Descriptive statistics; per-test medians need not sum to total median. This result uses the process count and order recorded above; the earlier isolated prototype is a separate experiment."},
           ...
           "after_vs_official_readme": {"total_ms": compare(official["compiled_total_ms"], after["total_ms"]["median"]),
                                         "primes_us": compare(official["compiled_primes_us"], after["tests_us"][PRIMES]["median"]),
                                         "list_us": compare(official["compiled_list_us"], after["tests_us"][LIST]["median"]),
...
for label, change in (("Total", changes["total_ms"]), ("Primes", changes["tests_us"][PRIMES]),
                      ("List Processing", changes["tests_us"][LIST]),
                      ("Lazy", changes["tests_us"][LAZY]), ("RBTree", changes["tests_us"][RBT])):
...
print(f"Official README: {official['compiled_total_ms']:.2f} ms total; "
      f"Primes compiled {official['compiled_primes_us'] / 1000:.5f} ms, "
      f"native optimized {official['native_optimized_primes_us'] / 1000:.3f} ms (different algorithm); "
      f"List Processing compiled {official['compiled_list_us'] / 1000:.5f} ms, "
      f"native optimized {official['native_optimized_list_us'] / 1000:.3f} ms.")
