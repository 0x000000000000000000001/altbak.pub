# FBIP via TAST : état au 16 septembre 2026
...
- [x] Corriger les annotations d'usage du compilateur : conserver `escapes`,
  ignorer les noms qualifiés par module, compter prudemment les gardes avec
  repli et ne plus fabriquer `(usageCount = 1, escapes = false)` sur tous
  les appels et constructeurs.
...
- [x] Rétablir les simplifications PBO bloquées par `UsageMeta`. La dépendance
  effective est `htdocs/purescript-backend-optimizer-purust`.
- [x] Invalider les compteurs source lors de l'évaluation de PBO, y compris
  son chemin pour grandes expressions. Conserver les types et `TypeApp`.
- [x] Normaliser les wrappers résiduels à l'entrée du générateur Rust pour
  que toutes les analyses de réemploi voient la même expression. Les
  décisions clone/move reposent sur la liveness finale (`alive`) ; un
  compteur source ne peut plus la contourner.
...
- [ ] Généraliser la variante Records dans le backend : champs primitifs
  ...
  Les types nécessaires existent déjà dans le TAST ; le prototype seul est
  terminé, cette transformation n'est pas intégrée.
...
Les gains RBTree et Records n'ont pas été mesurés ensemble.
...
annotation `usageCount`. Ne pas remplacer les contrôles dynamiques par une
