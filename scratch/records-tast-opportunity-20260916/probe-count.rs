...
static OP_READS:AtomicUsize=AtomicUsize::new(0);
...
include!("kernel-count-original.rs");
include!("kernel-count-scalar.rs");
...
fn main(){
 ...
 for (name,fun) in [("original",Test_Records_updateRec as fn(i64,Value)->Value),("scalar",scalar_replacement as fn(i64,Value)->Value)] {
  for n in [0,1,10000] {
   ...
   OP_READS.store(0,Ordering::Relaxed);OP_GETS.store(0,Ordering::Relaxed);OP_SETS.store(0,Ordering::Relaxed);ALLOCS.store(0,Ordering::Relaxed);DEALLOCS.store(0,Ordering::Relaxed);BYTES.store(0,Ordering::Relaxed);ON.store(true,Ordering::Relaxed);
   let r=fun(black_box(n),black_box(arg)); let vals=values(&r);drop(r);
   ON.store(false,Ordering::Relaxed);
   println!("operations {} n={} reads={} cloning_getters={} setters={}",name,n,OP_READS.load(Ordering::Relaxed),OP_GETS.load(Ordering::Relaxed),OP_SETS.load(Ordering::Relaxed));
