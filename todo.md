...
- **B14 remains yellow; B22 remains green: scalar record projections now borrow their path.** A local root and closed TAST rows prove each field down to an `Int`, `Number`, `Boolean` or `Char`, copied immediately. Owned results, open rows, opaque receivers and incompatible conversions keep their existing paths. Five alternating pairs of the actual generated full runners give **Records 0.625 → 0.353 ms (−0.272 ms, −43.5%)**, with non-overlapping ranges **0.620–0.628 / 0.352–0.356 ms**. Instrumented clones fall **110,000 → 20,000** for 10,000 updates, with **3 allocations/releases unchanged**. Total **12.455 → 12.084 ms** includes variation in unchanged RBTree code; the confidently attributable gain is **0.272 ms**, about **2.1%** of the official 12.98 ms suite. The rule also applies in five library modules and contains no benchmark names. [Scope, tests, counters and measurements](scratch/rust-record-borrow-integration-20260910/REPORT.md).
- **B13/B14 remain yellow: reconstruction guards can inspect the computed child.** A generic TAST proof now derives borrowed tag tests and dominated field projections from the helper's branches. The unique parent keeps its fields when the helper would only reconstruct them; complex branches receive the already-computed child once. Five alternating pairs of fresh generated full runners give **RBTree 15.498 → 11.528 ms**, **total 16.590 → 12.640 ms (−3.950 ms, −23.8%)**, with all five pairs favorable. **1,368,968 whole-payload extractions/reconstructions disappear**, with 100,001 allocations/releases unchanged. Shared/weak parents, opaque calls, conversions and multiple changed fields keep the existing paths; rotations still reconstruct. Persistent-version correctness passes, but performance gains there are not established. [Scope, cache verification, tests and measurements](scratch/rust-post-call-child-integration-20260910/REPORT.md).
- **B13/B14 remain yellow: a child updated through a closed native call.** A consumed unique ADT now retains its mutable borrow and writes only the changed child, when the TAST and a closed local call graph prove the case. Shared/weak parents, callbacks, opaque calls, conversions and rotations keep the existing paths. Five rotating blocks of actual generated full runners give **total 19.419 → 17.442 → 16.835 ms**: **1.977 ms** from direct reconstruction with the retained borrow, then **0.607 ms** from retaining unchanged fields. RBTree falls **18.309 → 15.741 ms**; **715,030 whole-payload extractions/reconstructions** disappear, with 100,001 allocations/liberations unchanged. [Scope, tests and measurements](scratch/rust-child-field-integration-20260909/REPORT.md).
- **B13: red → yellow.** A uniquely owned ADT can now update one scalar field while leaving the other fields in place. This first scalar-only step excluded calls, conversions and changes to multiple fields. The rule handles RBTree recoloration and four `Data.Time` setters. Five paired full runners measured **RBTree 18.237 → 17.644 ms**, **total 20.977 → 20.354 ms**. [Scope, tests and measurements](scratch/rust-recolor-field-20260909/REPORT.md).
...
- **B14 remains yellow; B22 remains green: reuse now follows a record path.** The same closed-TAST projection proof is applied at every level, following one child per record. All RHS values are evaluated before detaching the path; shared roots, children, leaves and captured old versions keep copying. Records has **10,003 → 3 allocations/liberations**, with **0.768 → 0.624 ms (−18.8%)** across five favorable pairs. Total **19.434 → 19.322 ms (−0.6%)** is a small observed difference within the variation. Only the initial three cells allocate on this unique path. [Scope, tests and measurements](scratch/rust-record-path-20260909/REPORT.md).
- **B14 remains yellow; B22 remains green: one nested child now reuses its cell.** Closed TAST rows and the same local root/field identify the update; all RHS values are staged before detaching the child. Shared roots, independently shared children and captured old versions keep copying. The generated Records kernel has **20,003 → 10,003 allocations/liberations**, with **0.861 → 0.767 ms (−10.9%)** across five paired full runners. Total **19.735 → 19.733 ms** remains within variation. That preceding one-child step still copied the leaf; the path extension above removes that copy. [Scope, tests and measurements](scratch/rust-record-child-20260909/REPORT.md).
...
These are separate experiments with their own baselines, not successive runs whose times can be subtracted. The latest integration passes **27 codegen tests, 16 TAST tests and all 14 runner results**, including a clean build and final generation. The independent record fixture covers four scalar types, eight sharing combinations, writes after reads, owned results, captures, opaque calls, short-circuiting and unwind. The official README, reread on 10 September, records compiled **Records 0.674 ms / RBTree 11.754 ms / total 12.98 ms**, versus **0.004 / 36.070 / 36.13 ms** in its last native column. The row order is retained: further ADT cases, read-only traversals and record representation still need their own proofs and measurements. Records no longer allocates during its unique update loop; its fields remain `Option<Value>` and some owned accesses remain. [Purust baby steps](../purust/purust/todo.md).
...
| **Reuse specialization: leave unchanged ADT fields in place** | **13 — B13, partial** | [ReuseFields](../purust/purust/src/Purust/ReuseFields.purs) updates one scalar field of a uniquely owned ADT, preserving unchanged fields in place; shared/weak cases retain the reconstruction fallback. [ChildCalls](../purust/purust/src/Purust/ChildCalls.purs) and [ChildUpdates](../purust/purust/src/Purust/ChildUpdates.purs) extend this to one child consumed by a closed native call, with a proven constructor-only helper branch and scalar/recursive layout. [ChildBranches](../purust/purust/src/Purust/ChildBranches.purs) also proves guards on the returned child, preserving ordered tag tests and dominated projections; complex branches consume that computed child exactly once. Opaque calls, conversions, multiple changed fields and rotations retain their existing paths. Fixed-field record setters belong to **B22**. **B42** is different: returning the original constructor when the replacement is already equal. |
| **Last-use moves / borrowing; propagation of uniqueness** | **14 — B14, partial**, also **B12** | Selected moves and borrows are integrated, including closed-record roots moved after replacement values at last use and a closed record path detached after all RHS values. Existing setters reuse unique roots/children/leaves with copying for shared versions; only the three initial cells allocate in Records. The rule follows one child per level, not every sibling branch. Closed record projections from locals can also borrow the entire path and immediately copy an Int/Number/Boolean/Char; escaping values and unproven paths keep owned getters. The new closed child-call path retains its mutable borrow across recursion and avoids its reconstruction retest; other ADT paths can still retest uniqueness, and read-only traversals can clone child pointers. |
