...
These are separate experiments with their own baselines, not successive runs whose times can be subtracted. The latest integration passes **23 codegen tests, 14 TAST tests and all 14 runner results**, including a clean build. The README remains the historical reference. The row order is retained: further ADT cases, read-only borrowing and record representation still need their own integration proofs and measurements. Records no longer allocates during its unique update loop; further work there requires evidence on residual field-access costs. [Purust baby steps](../purust/purust/todo.md).
