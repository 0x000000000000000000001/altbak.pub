...
These are separate experiments with their own baselines, not successive runs whose times can be subtracted. The latest integration passes **26 codegen tests, 15 TAST tests and all 14 runner results**, including a clean build and final regeneration. The official README, reread on 10 September, records compiled **RBTree 15.956 ms / total 17.12 ms**, versus **36.070 / 36.13 ms** in its last native column. The row order is retained: further ADT cases, read-only borrowing and record representation still need their own integration proofs and measurements. Records no longer allocates during its unique update loop; further work there requires evidence on residual field-access costs. [Purust baby steps](../purust/purust/todo.md).
