...
_ = __local_var_1_1
return gopurs_runtime.Apply(gopurs_runtime.Apply(Get_Effect_Console_log(), gopurs_runtime.Str(gopurs_runtime.Apply(Get_Data_Show_showIntImpl(), gopurs_runtime.Int(gopurs_runtime.RecordGet(gopurs_runtime.RecordGet(gopurs_runtime.RecordGet(Call_Test_Records_updateRec(__local_var_1_1.IntVal, Get_Test_Records_initial()), "b"), "d"), "f").IntVal)).StrVal())), gopurs_runtime.Value{})
})
}()
	})
