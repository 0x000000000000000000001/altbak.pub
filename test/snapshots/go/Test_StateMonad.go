	...
	for {
		...
		var __t1 gopurs_runtime.Value
		{
			if (v_0) == (0) {
				__t1 = gopurs_runtime.Func(func(s_1 gopurs_runtime.Value) gopurs_runtime.Value {
					return gopurs_runtime.RecordDict2("state", "val", s_1, Get_Data_Unit_unit())
				})
				goto end_branch_1
			} else {
		...
		}
		{
			...
			__t1 = gopurs_runtime.Func(func(s_2 gopurs_runtime.Value) gopurs_runtime.Value {
				return gopurs_runtime.Apply(Call_Test_StateMonad_chainModifications((v_0)-(1)), gopurs_runtime.RecordGet(gopurs_runtime.Apply(__local_var_1_0, s_2), "state"))
			})
		}
	end_branch_1:
		return __t1
...
runManyTimes:
	for {
		...
		{
			v_0_loop = (v_0) - (1)
			v1_1_loop = (v1_1) + (gopurs_runtime.RecordGet(gopurs_runtime.Apply(Call_Test_StateMonad_chainModifications(60), gopurs_runtime.Int(0)), "state").IntVal)
			continue runManyTimes
