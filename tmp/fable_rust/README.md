...
The extractor retains the declarations reachable from the 14 numeric kernels and the existing F# FFI members they need. The adapter removes unused local closure aliases only after checking that they are unreferenced. The pure runtime supports the generated curried function shapes without .NET reflection, uses the built-in F# `unbox`, and expresses literal type tests in a form Fable handles. Generated RBTree call adapters propagate their original exception instead of wrapping it in a .NET reflection exception. The adapter accepts two audited layouts: the older boxed `max`/`insert` pair and the newer typed `balance`/`ins`/`insert` trio. It checks their complete signatures and forwarding calls, preserves the successful call expressions verbatim, and records their hashes in the build manifest. These adjustments preserve successful benchmark calculations; they are not a general replacement for sharpurs' Effect/Aff runtime.

When present, the generated `sharpurs_int_mod` helper is retained verbatim alongside the compatibility runtime, including its Euclidean remainder and signed integer edge cases.

The typed LazyEvaluation builder uses an explicit thunk factory to capture the preceding thunk by value before the tail-call loop advances. This works around Fable's mutable tail-parameter capture and unit-function arity mismatch. An exact source-shape guard protects the adaptation: each step still allocates a non-memoizing thunk calling its predecessor and adding one, with the original depth and repetition count.
